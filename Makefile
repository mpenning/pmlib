...
# dynamic pmlib VERSION detection (via version str in pyproject.toml)
#
...
.PHONY: pypi
pypi:
	@echo "$(COL_CYAN)>> uploading pmlib pypi artifacts to pypi$(COL_END)"
	make clean
	# upgrade packaging infra and pmlib dependencies...
	make dep
...
.PHONY: repo-push
repo-push:
	@echo "$(COL_GREEN)>> git push and merge (w/o force) to pmlib main branch to github$(COL_END)"
	make ping
...
repo-push-force:
	@echo "$(COL_GREEN)>> git push and merge (w/ force) pmlib local main branch to github$(COL_END)"
	make ping
	-git checkout master || git checkout main # Add dash to ignore checkout fails
	# Now the main branch is checked out...
	THIS_BRANCH=$(shell git branch --show-current)  # assign 'main' to $THIS_BRANCH
...
.PHONY: repo-push-tag
repo-push-tag:
	@echo "$(COL_GREEN)>> git push (w/ local tag) pmlib local main branch to github$(COL_END)"
	git push origin +main
	git push --tags origin +main

.PHONY: repo-push-tag-force
repo-push-tag-force:
	@echo "$(COL_GREEN)>> git push (w/ local tag and w/ force) pmlib local main branch to github$(COL_END)"
	git push --force-with-lease origin +main
	git push --force-with-lease --tags origin +main

.PHONY: pylama
pylama:
	@echo "$(COL_GREEN)>> running pylama against pmlib$(COL_END)"
...
coverage-pytest:
	...
	cd tests;py.test --cov-report term-missing --cov=pmlib.py -s -v

.PHONY: pydocstyle
pydocstyle:
	# Run a numpy-style doc checker against all files matching pmlib/*py
	find pmlib/*py | xargs -I{} pydocstyle --convention=numpy {}
...
clean:
	...
	-rm -rf .eggs/
	-rm -rf .cache/
	-rm -rf build/ dist/ pmlib.egg-info/ setuptools*
